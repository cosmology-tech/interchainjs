...
export interface UseAppOptionsQuery<TData> extends VueQueryParams<AppOptionsResponse, TData> {
  request?: ReactiveAppOptionsRequest;
}
...
export const createRpcQueryHooks = (rpc: Ref<ProtobufRpcClient | undefined>) => {
  ...
  const useAppOptions = <TData = AppOptionsResponse,>({
    ...
    const queryKey = ["appOptionsQuery", queryService];
    if (request) {
      Object.values(request).forEach((val: any) => {
        queryKey.push(val);
      });
    }
    return useQuery<AppOptionsResponse, Error, TData>({
      queryKey,
      queryFn: () => {
        ...
        return queryService.value.appOptions(params);
      },
      ...options
    });
  };
