...
export interface UseStatusQuery<TData> extends VueQueryParams<StatusResponse, TData> {
  request?: ReactiveStatusRequest;
}
...
export const createRpcQueryHooks = (rpc: Ref<ProtobufRpcClient | undefined>) => {
  ...
  const useStatus = <TData = StatusResponse,>({
    ...
    const queryKey = ["statusQuery", queryService];
    if (request) {
      Object.values(request).forEach((val: any) => {
        queryKey.push(val);
      });
    ...
    return useQuery<StatusResponse, Error, TData>({
      ...
      queryFn: () => {
        ...
        return queryService.value.status(params);
      },
      ...options
    });
  };
