...
import { Message, Signer, TxRaw } from "@cosmonauts/cosmos-proto";
...
signer.register([["/cosmos.bank.v1beta1.MsgSend", MsgSend]]);
signer.on(chain.osmosis.rpc).by(Secp256k1Auth.fromMnemonic(seed.genesis));

async function run() {
  const signed = await signer.signMessages(messages);
