...
describe("Injective Chain transfer test", () => {
  ...
  beforeAll(async () => {
    ...
    directSigner = new DirectSigner(auth, [], rpcEndpoint, {
      prefix: "inj",
    });
    ...
    const signerFromDirectSecp256k1HdWallet =
      await DirectSecp256k1HdWallet.fromMnemonic(process.env.TEST_MNEMONIC, {
  ...
  test("transfer should be successful", async () => {
    const initialBalance = BigInt(
      (await client.getBalance(toAddress, "inj")).amount
    );
    const sendAmount = "1";
