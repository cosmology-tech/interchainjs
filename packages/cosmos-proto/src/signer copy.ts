...
import { AminoConverters, RegistryTypes } from "./types/amino";
...
export class Signer extends BaseSigner<QueryParser> {
  ...
  protected toSignature(sigObj: SigObj): Uint8Array {
    return new Uint8Array([...sigObj.r, ...sigObj.s]);
  }

  protected toSigObj(signature: Uint8Array): SigObj {
    return {
  ...
  protected _toTx(tx: AminoTxData<any>): Doc<any> {
    return {
  ...
  protected async _toOfflineTx<T>(data: Doc<T>): Promise<OfflineTxData<T>> {
  ...
  protected _signOffline(txData: OfflineTxData<WrapTypeUrl<any>>): TxRaw {
    ...
    const signature = this.signBytes(toBytes(this._toOfflineAminoTx(txData)));
  ...
  sign(tx: Doc<any>): Signed<Promise<TxRaw>> {
  ...
  signAmino(tx: AminoTxData<any>): Signed<Promise<TxRaw>> {
    const txData: Doc<any> = this._toTx(tx);
  ...
  signArbitrary(raw: Uint8Array): Signed<Uint8Array> {
    const signed = this.signBytes(raw);
    return {
      signed,
